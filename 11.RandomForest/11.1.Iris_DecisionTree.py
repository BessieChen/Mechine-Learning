...
if __name__ == "__main__":
    ...
    y = LabelEncoder().fit_transform(data[4]) #讲目标花的种类改成1，0标记。
    # 为了可视化，仅使用前两列特征
    x = x.iloc[:, :2]
    # x = x[[0,1]]
    ...
    # 决策树参数估计
    # min_samples_split = 10：如果该结点包含的样本数目大于10，则(有可能)对其分支.区别是，这个是没有分的时候大于10，下面是分完之后需要都大于10
    # min_samples_leaf = 10：若将某结点分支后，得到的每个子结点样本数目都大于10，则完成分支；否则，不进行分支
    model = DecisionTreeClassifier(criterion='entropy')  #可加：,max_depth= ,min_samples_split=
    model.fit(x_train, y_train)
    y_test_hat = model.predict(x_test)      # 测试数据
    print 'accuracy_score 测试集准确率:', accuracy_score(y_test, y_test_hat)

    y_train_pred = model.predict(x_train)  # 训练数据
    print 'accuracy_score: 训练集准确率', accuracy_score(y_train, y_train_pred)
